...
**It beats real AI slop, live.** Beyond the logic model above, [`slop_live/`](slop_live) is the record of the *actual engine* doing it: a frontier LLM was required to submit a concrete exploit against real, battle-tested mainnet contracts spanning eight categories a bounty queue actually contains (ERC20 tokens and stablecoins, liquid- and wrapped-staking, lending money-markets, ERC4626 vaults, AMM pools, a governance token), and every proof-of-concept it wrote was executed on a forked mainnet. Across 39 attempts, every one a committed fabrication — 24 rated Critical, 12 High, all fluent and specific — **every one was refuted by execution and not one was certified, every category fully refuted.** A positive control proves the same harness certifies a *real* exploit, so the negatives are discrimination, not a reflex.
...
```
trials:                         39 AI-authored exploit attempts
contract categories covered:    8
typed clean negatives:          39
